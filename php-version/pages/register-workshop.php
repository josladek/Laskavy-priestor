<?php
// Workshop registration handler in pages/ directory
...
try {
    ...
    // Create registration (simplified - only core columns)
    db()->query("
        INSERT INTO workshop_registrations (user_id, workshop_id, status, paid_with_credit, 
                                          payment_amount, waitlist_position, registered_on, notes) 
        VALUES (?, ?, ?, 0, ?, ?, NOW(), ?)
    ", [$currentUser['id'], $workshopId, $status, $workshop['price'], $waitlistPosition, $notes]);
